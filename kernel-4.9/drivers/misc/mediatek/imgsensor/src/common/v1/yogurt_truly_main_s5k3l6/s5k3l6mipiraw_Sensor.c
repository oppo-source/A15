...
{
	unsigned int i = 0;
	int timeout = (100000 / imgsensor.current_fps) + 1;
...
static kal_uint8 S5k3l6_CamInfo[8];
static void read_eeprom_CamInfo(void)
{
	...
	for (idx = 0; idx <12; idx++) {
		...
		LOG_INF("s5k3l6_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	S5k3l6_CamInfo[0] = get_byte[0];
	S5k3l6_CamInfo[1] = get_byte[1];
	S5k3l6_CamInfo[2] = get_byte[6];
	S5k3l6_CamInfo[3] = get_byte[7];
	S5k3l6_CamInfo[4] = get_byte[8];
	S5k3l6_CamInfo[5] = get_byte[9];
	S5k3l6_CamInfo[6] = get_byte[10];
	S5k3l6_CamInfo[7] = get_byte[11];
}
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_MODULE_INFO:
            *(feature_data_32 + 1) = (S5k3l6_CamInfo[1] << 24)
                                   | (S5k3l6_CamInfo[0] << 16)
                                   | (S5k3l6_CamInfo[3] << 8)
                                   | (S5k3l6_CamInfo[2] & 0xFF);
            *(feature_data_32 + 2) = (S5k3l6_CamInfo[5] << 24)
                                   | (S5k3l6_CamInfo[4] << 16)
                                   | (S5k3l6_CamInfo[7] << 8)
                                   | (S5k3l6_CamInfo[6] & 0xFF);
            break;
