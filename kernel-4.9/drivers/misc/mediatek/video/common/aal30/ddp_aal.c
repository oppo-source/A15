...
#ifdef ODM_WT_EDIT
static atomic_t g_aal_backlight_notified = ATOMIC_INIT(4095);
#else
...
//	pr_err("[lh_debug]%s bl_1024=%d\n",__func__,bl_1024);
	...
	if (bl_1024 == 0) {
		...
		if (atomic_read(&g_led_mode) == MT65XX_LED_MODE_CUST_LCM){
	//		pr_err("[ [lh_debug][%s]1bl_1024=%d\n",__func__,bl_1024);
			backlight_brightness_set_with_lock(bl_1024);
		}else{
		//	pr_err("[ [lh_debug][%s]2bl_1024=%d\n",__func__,bl_1024);
			backlight_brightness_set(bl_1024);
			}
	}
	...
	enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	//	pr_err("[ [lh_debug][%s]backlight_value1=%d\n",__func__,backlight_value);
	...
	//pr_err("[ [lh_debug][%s]backlight_value2=%d\n",__func__,backlight_value);
...
#ifdef ODM_WT_EDIT
	if (backlight_value > LED_4095) {
		backlight_value = LED_4095;
	}
#endif
	...
	*/
	...
	#endif
#ifdef ODM_WT_EDIT
	//pr_err("[ [lh_debug][%s]backlight_value3=%d\n",__func__,backlight_value);
	backlight_brightness_set(backlight_value);
#endif
