...
//#include <linux/oppo_healthinfo/oppo_fg.h>/*DELETE by shubin*/
...
void reqstats_record(struct request *req, unsigned nr_bytes, int fg);
static inline void iomonitor_record_reqstats(struct request *req,
					     unsigned int nr_bytes)
{
	req->req_tc = ktime_get();
	if (nr_bytes && req->req_tg.tv64 && req->req_td.tv64 && req->req_tc.tv64 ) {
