...
static int e4defrag_func(void *data)
{
	...
	bool done = true;
	struct key *key = NULL;
	DECLARE_WAIT_QUEUE_HEAD_ONSTACK(wait_queue);
	...
	key = e4defrag_install_session_keyring(1);
 loop:
	wait_event_interruptible_timeout(wait_queue, kthread_should_stop()
					 || freezing(current)
	...
	if (freezing(current)) {
		...
		e4defrag_msg("protecting online defrag thread as low power\n");
	} else {
...
static int e4defrag_start_thread(struct super_block *sb)
{
	...
	dfi->wq = &wait_queue;
	wait_event(wait_queue, dfi->task != NULL);
	return 0;
}
...
void e4defrag_exit(struct super_block *sb)
{
	...
	kobject_del(&dfi->kobj);
	memset(&dfi->kobj, 0, sizeof(dfi->kobj));
