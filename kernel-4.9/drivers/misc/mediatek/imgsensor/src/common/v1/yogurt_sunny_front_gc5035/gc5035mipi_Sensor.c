...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = YOGURT_SUNNY_FRONT_GC5035_SENSOR_ID,
...
static kal_uint16 read_cmos_sensor_sunny_otp(kal_uint32 addr)
{
    kal_uint16 get_byte=0;
...
static int read_gc5035_sunny_module_id(void)
{
	kal_uint16 gc5035_sunny_moduleid_id=0x0;
	gc5035_sunny_moduleid_id = read_cmos_sensor_sunny_otp(0x0000);
	printk("gc5035_sunny_moduleid_id = %d\n",gc5035_sunny_moduleid_id);
	return gc5035_sunny_moduleid_id;
}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == YOGURT_SUNNY_FRONT_GC5035_SENSOR_ID) {
#if ENABLE_GC5035_OTP
				/* Shipei.Chen@Cam.Drv, 20200414,  add for [add hardware_info for factory]*/
				if(read_gc5035_sunny_module_id() == 0x01)
				{
					*sensor_id = imgsensor_info.sensor_id;
					hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "Sunny");
					pr_debug("sunny gc5035 get_imgsensor_id *sensor_id = 0x%x\n",*sensor_id);
					//return ERROR_NONE;
				}
				else{
					*sensor_id = 0xFFFFFFFF;
					pr_debug("Sunny match fail,so truly");
...
UINT32 GC5035_SUNNY_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	cam_pr_debug("E\n");
