...
/*mingliang.Liang@Camera.Driver, 2020/08/18, add for google ARCode Feature verify*/
static kal_uint8 ov13b10_CamInfo[8];
static void read_eeprom_CamInfo(void)
{
	...
	for (idx = 0; idx <12; idx++) {
		...
		LOG_INF("ov13b10_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	ov13b10_CamInfo[0] = get_byte[0];
	ov13b10_CamInfo[1] = get_byte[1];
	ov13b10_CamInfo[2] = get_byte[6];
	ov13b10_CamInfo[3] = get_byte[7];
	ov13b10_CamInfo[4] = get_byte[8];
	ov13b10_CamInfo[5] = get_byte[9];
	ov13b10_CamInfo[6] = get_byte[10];
	ov13b10_CamInfo[7] = get_byte[11];
}
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_MODULE_INFO:
        *(feature_data_32 + 1) = (ov13b10_CamInfo[1] << 24)
                               | (ov13b10_CamInfo[0] << 16)
                               | (ov13b10_CamInfo[3] << 8)
                               | (ov13b10_CamInfo[2] & 0xFF);
        *(feature_data_32 + 2) = (ov13b10_CamInfo[5] << 24)
                               | (ov13b10_CamInfo[4] << 16)
                               | (ov13b10_CamInfo[7] << 8)
                               | (ov13b10_CamInfo[6] & 0xFF);
		break;
	default:
		break;
	}
