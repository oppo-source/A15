...
// Shouxing.Tang@ODM_WT.MM.AudioDriver.Machine 2020/06/24,add Yogurt audio speaker pa awnic 87359
#if defined(ODM_WT_EDIT) && defined(CONFIG_SND_SOC_AW87359)
...
//add for reduce aw87359 output voltage
extern void aw87359_audio_spk_low_voltage_status(bool bStatus);
extern int aw87359_spk_low_voltage_status;
#endif
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
		...
		//Shouxing.Tang@ODM_WT.MM.AudioDriver.Machine 2020/06/24,add Yogurt audio speaker pa awnic 87359
		if(is_voice_call)
		    aw87359_audio_abspk(); //for voice call
		else
		    aw87359_audio_dspk(); // for playback
#else
...
#ifdef ODM_WT_EDIT
//Shouxing.Tang@ODM_WT.mm.audiodriver.HAL, 2020/07/04,add call state for awinic speaker pa
static int Aw87359_Scene_Get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = is_voice_call;
	return 0;
}
static int Aw87359_Scene_Set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
#if defined(ODM_WT_EDIT) && defined(CONFIG_SND_SOC_AW87359)
/* Shouxing.Tang@ODM_WT.mm.audiodriver.Codec, 2020/07/24, add for reduce aw87359 output voltage */
static int ext_amp_low_voltage_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.integer.value[0] = aw87359_spk_low_voltage_status;
        pr_info("%s: aw87359_spk_low_voltage_status = %d\n", __func__, aw87359_spk_low_voltage_status);
        return 0;
}

static int ext_amp_low_voltage_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
        if (ucontrol->value.integer.value[0] == aw87359_spk_low_voltage_status)
                return 1;
        if (ucontrol->value.integer.value[0] == 1) {
                aw87359_audio_spk_low_voltage_status(1);
        } else {
                aw87359_audio_spk_low_voltage_status(0);
        }
...
#ifdef ODM_WT_EDIT
	//Shouxing.Tang@ODM_WT.mm.audiodriver.HAL, 2020/07/04,add call state for awinic speaker pa
	SOC_ENUM_EXT("Aw87359_Scene_Voice", Audio_DL_Enum[11],
		     Aw87359_Scene_Get,
		     Aw87359_Scene_Set),
#endif
#if defined(ODM_WT_EDIT) && defined(CONFIG_SND_SOC_AW87359)
    /* Shouxing.Tang@ODM_WT.mm.audiodriver.Codec, 2020/07/24,add for reduce aw87359 output voltage */
    SOC_ENUM_EXT("Ext_AMP_LOW_VOLTAGE", Audio_DL_Enum[15], ext_amp_low_voltage_get,
