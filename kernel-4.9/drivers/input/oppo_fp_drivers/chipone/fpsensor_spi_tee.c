...
/*
        fpsensor->fp_pwr_high = pinctrl_lookup_state(fpsensor->pinctrl1, "fpsensor_finger_pwr_high");
        if (IS_ERR(fpsensor->fp_pwr_high)) {
            ret = PTR_ERR(fpsensor->fp_pwr_high);
            fpsensor_debug(ERR_LOG, "fpsensor Cannot find fp pinctrl fp_pwr_high!\n");
            return ret;
        }
        fpsensor->fp_pwr_low = pinctrl_lookup_state(fpsensor->pinctrl1, "fpsensor_finger_pwr_low");
        if (IS_ERR(fpsensor->fp_pwr_low)) {
            ret = PTR_ERR(fpsensor->fp_pwr_low);
            fpsensor_debug(ERR_LOG, "fpsensor Cannot find fp pinctrl fp_pwr_low!\n");
            return ret;
        }
*/
    } else {
...
#endif
{
    ...
    struct platform_device *pdev = NULL;
    int ret = 0;
//	struct regulator *finger_regulator;
...
*/
    ...
    if (node) {
        ...
        fpsensor_dev->fp_pwr_high = pinctrl_lookup_state(fpsensor_dev->pinctrl1, "fpsensor_finger_pwr_high");
        ...
        fpsensor_dev->fp_pwr_low = pinctrl_lookup_state(fpsensor_dev->pinctrl1, "fpsensor_finger_pwr_low");
        ...
        if (IS_ERR(fpsensor_dev->fp_cs_gpio)) {
            ...
            fpsensor_debug(ERR_LOG, "fpsensor_dev Cannot find fp pinctrl fp_cs_gpio!\n");
            return ret;
        }
        ...
        if (IS_ERR(fpsensor_dev->fp_cs_func)) {
            ...
            fpsensor_debug(ERR_LOG, "fpsensor_dev Cannot find fp pinctrl fp_cs_func!\n");
            return ret;
        }
    } else {
        fpsensor_debug(ERR_LOG,"fpsensor Cannot find node!\n");
...
*/
    ...
    if(2 == sim_val){ 
	printk("%s, Need to register chipone FP driver\n", __func__);
    } else {
        printk("%s, Don't need to register chipone FP driver\n", __func__);
        return -1;
    }
