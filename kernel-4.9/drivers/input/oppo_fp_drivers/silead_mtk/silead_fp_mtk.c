...
/*
   ...
   if ( fp_dev->pin.finger_cs_func ) {
           err = pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.finger_cs_func);
           if(err)
              LOG_MSG_DEBUG(ERR_LOG, "%s: finger_cs_func error ret:%d \n", __func__, err);
    }
#endif
...
#ifdef FINGER_CS_CONCTROL
    if ( fp_dev->pin.finger_cs_gpio ) {
        pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.finger_cs_gpio);/*output cs low */
        
        LOG_MSG_DEBUG(ERR_LOG, "%s: finger_cs_gpio\n",__func__);
    }
#endif
    if ( fp_dev->pin.pins_power_l ) {
        err = pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.pins_power_l);
        if(err)
             LOG_MSG_DEBUG(ERR_LOG, "%s: power supply error ret:%d \n", __func__, err);
    }
#endif /* BSP_SIL_POWER_SUPPLY_PINCTRL */
...
static void silfp_power_deinit(struct silfp_data *fp_dev)
{
    ...
    /* Power control by GPIOs */
    if ( fp_dev->pin.pins_power_l ) {
        pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.pins_power_l);/*disable vdd */
        
        LOG_MSG_DEBUG(ERR_LOG, "%s: power off\n",__func__);
    }
#ifdef FINGER_CS_CONCTROL
    if ( fp_dev->pin.finger_cs_gpio ) {
        pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.finger_cs_gpio);/*output cs low */
    ...
    if ( fp_dev->pin.pins_rst_l ) {
		...
		LOG_MSG_DEBUG(ERR_LOG, "%s: pins_rst_l\n",__func__);

    }
	gpio_direction_output(fp_dev->pin.power_vdd_en, 1);//disable power for fpc finger
...
#ifdef FINGER_CS_CONCTROL
	fp_dev->pin.finger_cs_gpio = NULL;
#endif
   // fp_dev->pin.pins_vddio_h = NULL;
#endif /* BSP_SIL_POWER_SUPPLY_PINCTRL */
...
#ifdef FINGER_CS_CONCTROL	
     fp_dev->pin.power_vdd_en=of_get_named_gpio(pdev->dev.of_node, "fpc,vdd-en", 0);
     if (!gpio_is_valid(fp_dev->pin.power_vdd_en)) {
            pr_info("fp_dev->pin.power_vdd_e is invalid.\n");
            return -1;
        }
#endif
...
#ifdef FINGER_CS_CONCTROL
	fp_dev->pin.finger_cs_gpio= pinctrl_lookup_state(fp_dev->pin.pinctrl, "finger_cs_gpio");
    if (IS_ERR_OR_NULL(fp_dev->pin.finger_cs_gpio)) {
        fp_dev->pin.finger_cs_gpio = NULL;
        LOG_MSG_DEBUG(ERR_LOG, "%s can't find silfp finger_cs_gpio\n", __func__);
        // Ignore error
    }
	
	fp_dev->pin.finger_cs_func= pinctrl_lookup_state(fp_dev->pin.pinctrl, "finger_cs_func");
    if (IS_ERR_OR_NULL(fp_dev->pin.finger_cs_func)) {
        fp_dev->pin.finger_cs_func = NULL;
        LOG_MSG_DEBUG(ERR_LOG, "%s can't find silfp finger_cs_gpio\n", __func__);
        // Ignore error
    }
#endif
