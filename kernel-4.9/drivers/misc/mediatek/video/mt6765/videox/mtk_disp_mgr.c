        ...
        struct device_attribute *attr, const char *buf, size_t num)
{
	...
	if(!strcmp(_get_context()->plcm->drv->name,"oppo17321_boe_td4310_1080p_dsi_vdo")) {
		return num;
	}
...
/*hongwu.wei @PSW.MM.Display.LCD.Machine, 2020/07/03,add checklist feature*/
/* Rongchun.Zhang@PSW.MM.Display.LCD.Machine, 2017/09/14, Add for outdoor mode */
	ret = device_create_file(dev, &dev_attr_outdoorbl);
	if (ret < 0) {
		printk("%s outdoorbl device create file failed!\n", __func__);
	}
#endif
