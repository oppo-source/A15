...
#else
		unsigned int dev = bio ? bio->bi_bdev->bd_dev : 0;
#endif
				    ...
				    struct request *rq)
{
	if (io_trace_this->enable) {
		...
		io_trace_global_log(BLOCK_RQ_INSERT_TAG,
				    ...
				    -1000, NULL, 0);
#else
        unsigned long sector = (rq->cmd_type == REQ_TYPE_BLOCK_PC) ? 0 : blk_rq_pos(rq);
		io_trace_global_log(BLOCK_RQ_INSERT_TAG,
...
#else
        unsigned long sector = (rq->cmd_type == REQ_TYPE_BLOCK_PC) ? 0 : blk_rq_pos(rq);
		io_trace_global_log(BLOCK_RQ_ISSUE_TAG, sector,
				    blk_rq_bytes(rq), 0, rw_type, q, MAJOR(dev),
				    MINOR(dev), -1000, NULL, 0);
#endif
	}
...
static int test_task_ux(struct task_struct *p)
{
	return p->static_ux;
}
