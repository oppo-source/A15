...
//unsigned long cabc_gamma_flag =1;
        ...
        struct device_attribute *attr, const char *buf, size_t num)
{
	...
	//	if(0==cabc_gamma_flag){
		//	primary_display_set_gamma_mode(0);
		//	cabc_gamma_flag = 1;
		//	pr_err("%s primary_display_set_gamma_mode to higher light\n", __func__);
	//	}
	}else if (CABC_SWITCH_GAMMA == CABC_mode){
		cabc_sun_flag =1;
		cabc_true_mode =0;
	//	if(1==cabc_gamma_flag){
		//	primary_display_set_gamma_mode(1);
		//	cabc_gamma_flag = 0;
		//	pr_err("%s primary_display_set_gamma_mode to lower light\n", __func__);
	//	}
