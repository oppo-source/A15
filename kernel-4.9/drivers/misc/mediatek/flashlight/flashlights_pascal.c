...
#define PASCAL_LEVEL_NUM 26
#define PASCAL_LEVEL_TORCH 7
static int g_duty_array[PASCAL_LEVEL_NUM] = {4, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96, 100};
...
static int pascal_enable(void)
{
	...
	/* Shipei.Chen@Cam.Drv, 20200515,  modify for torch to gpio driver!*/
	if (g_flash_duty == PASCAL_LED_MODE_DUTY) {//led mode
	...
	} else if (g_flash_duty == PASCAL_TORCH_MODE_DUTY) {//torch mode
		pascal_pinctrl_set(PASCAL_PINCTRL_PIN_FLASH_EN,0);
		...
		pr_debug("PASCAL Flash %s g_flash_duty = %d,%d\n",__FUNCTION__,g_flash_duty,tempPWM);
