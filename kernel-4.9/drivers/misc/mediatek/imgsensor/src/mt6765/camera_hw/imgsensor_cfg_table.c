...
int yogurt_project(void){//// yogurt 1.yogurt 2 yogurte 3 yogurt 11
	...
	printk("yogurt_project projectvalue = %d\n",projectvalue);
...
int pascal_project(void){//// pascald 1.pascal 2 pascale 3
	...
	if(get_Operator_Version() == 131 ||get_Operator_Version() == 132
	    ...
	    projectvalue = 2;
	}else if(get_Operator_Version() == 136 ||get_Operator_Version() == 138
	    ||get_Operator_Version() == 139) {
...
#endif
//wenhui.chen@Cam.Drv, 20200416, add for pascal camera drvier porting
#if defined(PASCAL_HLT_WIDE_OV8856)
    {
        SENSOR_DRVNAME_PASCAL_HLT_WIDE_OV8856,
        {
            {RST, Vol_Low, 1},
            {DOVDD, Vol_1800, 1},
            {AVDD, Vol_2800, 1},
...
#if defined(YOGURT_HLT_FRONT_GC5035)
    {
        SENSOR_DRVNAME_YOGURT_HLT_FRONT_GC5035,
        {
            {PDN, Vol_Low, 0},
            {RST, Vol_Low, 3},
            {DOVDD, Vol_1800, 1},
...
#endif
#if defined(YOGURT_SUNNY_FRONT_GC5035)
    {
        SENSOR_DRVNAME_YOGURT_SUNNY_FRONT_GC5035,
        {
            {PDN, Vol_Low, 0},
            {RST, Vol_Low, 3},
            {DOVDD, Vol_1800, 1},
