...
/*Binbo.Song@ODM.BSP.TP 2020/07/31 TP changer adapt*/
...
#ifdef CONFIG_OPPO_SMART_CHARGER_SUPPORT
		case POWER_SUPPLY_PROP_COOL_DOWN:
			charger_xlog_printk(CHG_LOG_CRTI, "wenboyu cool_down [0x%x].\n", val->intval);
...
#ifdef ODM_WT_EDIT
/*Shouli.Wang@ODM_WT.BSP.CHG 2020/07/17,extend normal temp range*/
				if(chip->charge_normal_temp_high_flag)
					charging_current = CHARGER_NORMAL_TEMP_HIGH_CURRENT;
				else
#endif
...
static void oppo_chg_set_input_current_limit(struct oppo_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on) && !chip->is_double_charger_support   && !chip->cool_down) {
		if (chip->led_temp_status == LED_TEMP_STATUS__HIGH) {
		...
		if ((chip->chg_ctrl_by_camera) && (chip->camera_on) && (current_limit > chip->limits.input_current_camera_ma) && !chip->cool_down) {
			current_limit = chip->limits.input_current_camera_ma;
			voter = CAMERA;
		}

	} else if ((chip->chg_ctrl_by_camera) && (chip->camera_on) && (current_limit > chip->limits.input_current_camera_ma) && !chip->cool_down) {
		current_limit = chip->limits.input_current_camera_ma;
		voter = CAMERA;
	}
...
/*Shouli.Wang@ODM_WT.BSP.CHG 2020/07/17,extend normal temp range*/
	if(chip->tbatt_status==BATTERY_STATUS__NORMAL){
		if(batt_temp >= CHARGER_NORMAL_TEMP_HIGH){
			if(!chip->charge_normal_temp_high_flag){
				chip->charge_normal_temp_high_flag = true;
				oppo_chg_set_charging_current(chip);
			}
		} else {
			if(batt_temp <= CHARGER_NORMAL_TEMP_HIGH_MINUS && chip->charge_normal_temp_high_flag){
				chip->charge_normal_temp_high_flag = false;
				oppo_chg_set_charging_current(chip);
			}
		}
	}
...
{
	chip->cool_down = 1;
	charger_xlog_printk(CHG_LOG_CRTI, "wenboyu set cool_down = [%d].val=0x%x\n", chip->cool_down,val);
	switch (val)
	{
		case SMART_CHARGER_5V500MA:
		...
		case SMART_CHARGER_5V1500MA:
			...
			break;
		case SMART_CHARGER_5V1700MA:
			chip->limits.input_current_charger_ma    = OPPO_CHG_1700_CHARGING_CURRENT;
			chip->limits.pd_input_current_charger_ma = OPPO_CHG_1700_CHARGING_CURRENT;
			chip->limits.qc_input_current_charger_ma = OPPO_CHG_1700_CHARGING_CURRENT;
			break;
		default:
