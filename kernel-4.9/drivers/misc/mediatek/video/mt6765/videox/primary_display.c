...
#define FFL_END_LEVEL ((LED_4095) * 23 / 100)
#define FFL_UPRATE (1)
...
#ifdef ODM_WT_EDIT
 //Hao.Liang@ODM_WT.MM.Display.Lcd, 2020/06/1, LCD backlight need delay serial times after resume lp11
...
int _set_cabc_mode_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		...
		disp_lcm_oppo_set_lcm_cabc_cmd(pgc->plcm, cmdq_handle_lcm_cabc, level);

		_cmdq_flush_config_handle_mira(cmdq_handle_lcm_cabc, 0);
...
int _set_gamma_mode_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		...
		cmdqRecReset(cmdq_handle_lcm_cmd);
			...
			disp_lcm_oppo_set_lcm_gamma_cmd(pgc->plcm, NULL, level);
		...
		if (primary_display_cmdq_enabled()) {
			if (primary_display_is_video_mode()) {
				mmprofile_log_ex(
					...
					MMPROFILE_FLAG_PULSE, 0, 7);
				#ifdef ODM_WT_EDIT
				//Hao.Liang@ODM_WT.MM.Display.Lcd, 2020/07/28, Modify always no backlight when aging test
				//pr_err("[ [lh_debug][%s]1level=%d\n",__func__,level);
				_set_backlight_by_cmdq(level);
				#else
				disp_lcm_set_backlight(pgc->plcm, NULL, level);
				#endif
			} else {
				//pr_err("[ [lh_debug][%s]2level=%d\n",__func__,level);
				_set_backlight_by_cmdq(level);
			}
			atomic_set(&delayed_trigger_kick, 1);
		} else {
